...
const AppContextProvider = (props) => {
    ...
    const generateImage = async(prompt) => {
        try {
            ...
            // Ensure that user is logged in and user.id is available
            console.log("Prompt:", prompt);
            console.log("User ID:", user._id); // Ensure user.id is correct

            const { data } = await axios.post(backendUrl + '/api/image/generate-image', { prompt, userId: user._id }, { headers: { Authorization: `Bearer ${token}` } });
